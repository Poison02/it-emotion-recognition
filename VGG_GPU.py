...
path_train = 'face_images/fer_train_1'
path_valid = 'face_images/fer_test_1'
...
def train(model, device, dataset, optimizer, epoch):
    ...
    print("Epoch {} Loss {:.4f} Accuracy {}/{} ({:.0f}%)".format(epoch, loss, correct, len(data_train),
                                                                 100 * correct / len(data_train)))
...
def valid(model, device, dataset):
    ...
    valid_ac.append(correct / len(data_valid))
    valid_loss.append(loss.item())
    print("Test Loss {:.4f} Accuracy {}/{} ({:.0f}%)".format(loss, correct, len(data_valid),
                                                             100. * correct / len(data_valid)))
...
def RUN():
    for epoch in range(1, EPOCH + 1):
        ...
        # 保存模型
        torch.save(model, 'model/model_vgg_GPU.pkl')
