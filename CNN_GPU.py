...
def train(train_dataset, val_dataset, batch_size, epochs, learning_rate, wt_decay):
    ...
    for epoch in range(epochs):
        ...
        for images, emotion in train_loader:
            ...
            # 误差计算
            loss_rate = loss_function(output, emotion.to(device))
            # 误差的反向传播
            loss_rate.backward()
            # 更新参数
            optimizer.step()
...
def main():
    # 数据集实例化(创建数据集)
    train_dataset = FaceDataset(root='face_images/fer_train')
    val_dataset = FaceDataset(root='face_images/fer_test')
    # 超参数可自行指定
    model = train(train_dataset, val_dataset, batch_size=128, epochs=100, learning_rate=0.01, wt_decay=0)
    # 保存模型
    torch.save(model, 'model/model_cnn_GPU(4).pkl')
