...
classification_model_path = 'model/model_resnet_GPU.pkl'
...
img = cv2.imread(r"D:\it-test-img\test.jpg")

frame = img[:, ::-1, :] # 水平翻转，符合自拍习惯
frame = frame.copy()
...
for (x, y, w, h) in faces:
    ...
    new_new_face = new_face.float().requires_grad_(False).to(device)

    # 调用我们训练好的表情识别模型，预测分类
    emotion_arg = np.argmax(emotion_classifier.forward(new_new_face).detach().cpu().numpy())
    emotion = emotion_labels[int(emotion_arg)]
